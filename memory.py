...
def load_convo():
    files = os.listdir('./memory_storage/chat_logs')
    files = [i for i in files if '.json' in i]  # filter out any non-JSON files
    result = list()
    for file in files:
        data = load_json('./memory_storage/chat_logs/%s' % file)
        result.append(data)
...
def load_memory():
    files = os.listdir('./memory_storage/notes')
    files = [i for i in files if '.json' in i]  # filter out any non-JSON files
    result = list()
    for file in files:
        data = load_json('./memory_storage/notes/%s' % file)
        result.append(data)
    return result
...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.0, top_p=1.0, tokens=600, freq_pen=0.0, pres_pen=0.0, stop=['USER:', 'Jarvis:']):
    ...
    while True:
        try:
            ...
            save_file('./memory_storage/memories/%s' % filename, prompt + '\n\n==========\n\n' + text)
            return text
...
def summarize_memories(memories):  # summarize a block of memories into one payload
    ...
    block = block.strip()
    prompt = open_file('./memory_storage/prompt_notes.txt').replace('<<INPUT>>', block)
    ...
    save_json('./memory_storage/notes/%s' % filename, info)
    return notes, vector
